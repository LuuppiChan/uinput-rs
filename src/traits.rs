pub trait IntoKeyName {
    /// Converts the given key to the corresponding string representation.
    ///
    /// # Examples
    /// ```rust
    /// use uinput_rs::key_codes::{KEY_ESC, KEY_A, KEY_ENTER};
    ///
    /// assert_eq!(KEY_ESC.into_key_name(), "KEY_ESC");
    /// assert_eq!(KEY_A.into_key_name(), "KEY_A");
    /// assert_eq!(KEY_ENTER.into_key_name(), "KEY_ENTER");
    /// ```
    fn into_key_name(self) -> &'static str;
}

impl IntoKeyName for u16 {
    fn into_key_name(self) -> &'static str {
        match self {
            0 => "KEY_RESERVED",
            1 => "KEY_ESC",
